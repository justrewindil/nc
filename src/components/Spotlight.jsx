    ...
    <Reveal>
      <div className="spotlight" style={{ backgroundImage: `url(${bd})` }}>
        <div className="spot-inner">
          <span className="spot-tag">Featured</span>
          {logo
          ...
          <div className="spot-meta">
            ...
            <span className="chip">{type === 'tv' ? 'TV Series' : 'Movie'}</span>
          </div>
          <p className="spot-ov">{item.overview}</p>
          <div className="hero-btns">
            <button className="btn-play" onClick={() => openPlayer(item.id, type)}><Play size={16} fill="#fff" stroke="none" /> Watch Now</button>
            <button className="btn-info" onClick={() => openDetail(item.id, type)}><Info size={16} /> More Info</button>
          </div>
        </div>
      </div>
    </Reveal>
